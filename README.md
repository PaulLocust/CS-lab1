...
├── scripts/
│   ├── api-test.sh                 # обход всех эндпоинтов с проверкой кодов ответа
│   ├── smoke-test.sh               # проверка защиты через curl (SQLi, XSS, перебор)
...
```bash
./mvnw test                       # 43 автотеста
./mvnw verify                     # тесты + SAST (SpotBugs + FindSecBugs)
./mvnw -Psecurity-scan verify     # + SCA (OWASP Dependency-Check)
bash scripts/api-test.sh          # обход всех эндпоинтов с проверкой кодов
bash scripts/smoke-test.sh        # проверка защиты: SQLi, XSS, перебор паролей
```
...
**43 автотеста** (JUnit 5 + MockMvc). 23 из них (`SecurityDefenceTest`, `HtmlSanitizerTest`,
...
| `DataApiTest` | 13 | `401` без токена, `200` с токеном, создание записи, `403` при удалении чужой записи, `204` для автора, `403` на админский эндпоинт для обычного пользователя, корректные коды для кривых запросов (400/405/415 вместо 500) |
...
Ручная проверка выполнялась через `curl` и Postman:

* [scripts/api-test.sh](scripts/api-test.sh) — обходит все восемь эндпоинтов и сверяет коды
  ответа, включая ошибочные сценарии; 59 проверок, возвращает ненулевой код при провале;
* [scripts/smoke-test.sh](scripts/smoke-test.sh) и [scripts/smoke-test.ps1](scripts/smoke-test.ps1)
  — демонстрация защиты: SQL-инъекции, XSS, подделка токена, перебор паролей;
* [postman/CS-lab1.postman_collection.json](postman/CS-lab1.postman_collection.json) — коллекция
  с автотестами внутри запросов;
* вывод реальных прогонов — в [docs/api-examples.md](docs/api-examples.md).
